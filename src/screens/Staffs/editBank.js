...
import {updateBankAccount } from "../../Redux/Actions/bankAccountAction";
...
const EditBank = ({ navigation,route }) => {
  ...
  const unitCode = "WAY4";
  const { updatedBankAccount, isLoading, error } = useSelector(state => state.updatebankAccountReducer);
  const handleSave = async () => {
    ...
    const update_bankPayload = {unitCode, companyCode,id:bankId,name: bankName,accountType: bankType,accountNumber,branchName,branchId:1,ifscCode:IFSC,phoneNumber,address,accountName:accounbtHolderName,totalAmount:amount};
   await dispatch(updateBankAccount(update_bankPayload));
  ...
  useEffect(() => {
    if (bankDetails) {
      setBankType(bankDetails.accountType);
      setAccounbtHolderName(bankDetails.accountName);
      setAccountNumber(bankDetails.accountNumber);
      setAddress(bankDetails.address);
      setAmount(bankDetails.totalAmount);
      setBankName(bankDetails.name);
      setBranchName(bankDetails.branchName);
      setIFSC(bankDetails.ifscCode);
      setPhoneNumber(bankDetails.phoneNumber);
      setBankId(bankDetails.id)
    }   
  }, [bankDetails]);
  return (
