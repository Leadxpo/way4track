...
const InvoiceDetails = ({ navigation, route }) => {
  ...
  const { invoiceDetails } = route.params;  // Sample address data
  return (
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.body}>
        ...
        <Card style={styles.card}>
          <Card.Content>
            ...
            <View style={styles.invoiceInfo}>
              <Text>Date: {invoiceDetails.invoiceDate.split('T')[0]}</Text>
              <Text>Due Date: {invoiceDetails.expireDate}</Text>
            </View>
          </Card.Content>
        </Card>
        ...
        <DataTable>
          ...
          <DataTable.Row>
                  <DataTable.Cell style={{}}>Total Invoice Amount</DataTable.Cell>
                  <DataTable.Cell numeric>₹ {invoiceDetails.totalAmount} /-</DataTable.Cell>
                </DataTable.Row>
        </DataTable>

        {/* Buttons */}
        <View style={styles.buttonContainer}>
          <Button mode="contained" style={styles.buttonOrange} onPress={()=>navigation.navigate('AddInvoices',{invoiceDetails:invoiceDetails})}>{`sendInvoice`}</Button>
          </View>
      </ScrollView>
    </View>
  );
