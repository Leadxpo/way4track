...
const RecieptDetails = ({ navigation, route }) => {
  
  const { recieptDetails } = route.params;  // Sample address data
  return (
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.body}>
        ...
        <Card style={styles.card}>
          <Card.Content>
            ...
            <View style={styles.invoiceInfo}>
              <Text>Date: {recieptDetails.recieptDate.split('T')[0]}</Text>
              <Text>Due Date: {recieptDetails.expireDate}</Text>
            </View>
          </Card.Content>
        </Card>
        ...
        <DataTable>
          ...
          {
            recieptDetails.products.map((item, index) => {
              return (
          ...
          <DataTable.Row>
                  <DataTable.Cell style={{}}>Total Reciept Amount</DataTable.Cell>
                  <DataTable.Cell numeric>₹ {recieptDetails.totalAmount} /-</DataTable.Cell>
                </DataTable.Row>
        </DataTable>

        {/* Buttons */}
        <View style={styles.buttonContainer}>
          <Button mode="contained" style={styles.buttonOrange} onPress={()=>navigation.navigate('AddInvoices',{recieptDetails:recieptDetails})}>{`sendReciept`}</Button>
          </View>
      </ScrollView>
    </View>
  );
