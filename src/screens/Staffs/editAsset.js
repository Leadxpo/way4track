...
import {updateAssert } from '../../Redux/Actions/assertAction';
...
const EditAsset = ({ navigation, route }) => {
  ...
  const [voucherId, setVoucherId] = useState(assetDetails.voucherId.voucherId);
  ...
  useEffect(() => {
    ...
    if (assetDetails) {
      setId(assetDetails.id || '');
      setAssetPhoto(assetDetails.assetPhoto || null); 
      setAssetType(assetDetails.assetType || '');
      setQuantity(assetDetails.quantity.toString() || '');
      setDescription(assetDetails.description || '');
      setPurchaseDate(assetDetails.purchaseDate || '');
      setVoucherId(assetDetails.voucherId.voucherId || null);
      setBranchId(assetDetails.branchId.id || '');
      setBranchName(assetDetails.branchId.branchName || '');
    }
  }, [assetDetails]);  // Runs whenever assetDetails changes
  ...
  const getVoucherData = React.useCallback(() => {
    if (!voucherId) {
  ...
  },[voucherId, companyCode, unitCode, dispatch]);
  ...
  const handleSave = () => {
    ...
    dispatch(updateAssert(payload)); // Uncomment when the action is ready
  };
  ...
  return (
    <View style={styles.container}>
      <ScrollView style={{ padding: 6 }}>
        ...
        />
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', flex: 1, alignSelf: 'center' }}>
          <Text style={{ fontWeight: 700, fontSize: 18,color:"#333333" }}>{branchName}</Text>
          <Button mode="contained" onPress={()=>setIsChangeBranch(true)} buttonColor='#4CAF50' textColor='#f3f3f3' labelStyle={{ fontSize: 15, fontWeight: '700' }} style={[styles.saveButton, { flex: 1, height: 45, marginHorizontal: 5 }]}>
            Change Branch
          </Button>
        </View>

        {isBranchChange && <BranchesDropdown dropdownStyles={styles} onBranchChange={setBranchId} />}
