...
const LeadPayment = ({ navigation }) => {
  ...
  const workStatusMap = useMemo(() => ({
    pendingLeadPayment: filteredData?.filter(item => item.leadStatus === "paymentPending"),
    partiallyPaidLead: filteredData?.filter(item => item.leadStatus === "partiallyPaid"),
  ...
  const getStatusColor = (status) => {
    switch (status) {
      ...
      case "partiallyPaid": return "#ffeeec";
          ...
          ListEmptyComponent={() => (
            <View style={styles.emptyContainer}>
              <Card style={styles.emptyCard}>
                <Text style={styles.emptyText}>No Leads Generated Yet Yet</Text>
              </Card>
            </View>
          )}
        />
